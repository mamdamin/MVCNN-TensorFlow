...
def _conv(name, in_ ,ksize, strides=[1,1,1,1], padding=DEFAULT_PADDING, group=1, reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse) as scope:
        ...
        conv = tf.nn.leaky_relu(conv, name=scope.name)
...
## Amin
def _fc_Last(name, in_, outsize, dropout=1.0, reuse=False):
    with tf.variable_scope(name, reuse=reuse) as scope:
        # Move everything into depth so we can perform a single matrix multiply.
...
def loss(fc8, labels):
    ...
    indices = labels,
    depth=2)
    y_hat= tf.nn.softmax(fc8)
    l = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=fc8)
    #l = tf.losses.hinge_loss(labels=oneHotLabels, logits=fc8)
    ##Amin
    #if tf.reduce_mean(fc8)==0:
    #class1s = tf.reduce_mean(fc8)
